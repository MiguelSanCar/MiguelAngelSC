...
public class Main {
    public static void main(String[] args) throws InterruptedException {
        ...
        if(elegirModo.equals("1")){
            ...
            System.out.println("Historia - Actualidad - Ciencia");
            do{
                tematica = teclado.next();
                tematica = tematica.toLowerCase();
                if(tematica.equals("historia") == false && tematica.equals("actualidad") == false && tematica.equals("ciencia") == false){
                    System.out.println("Por favor, escoja una de las opciones");
                }
            }while(tematica.equals("historia") == false && tematica.equals("actualidad") == false && tematica.equals("ciencia") == false);

            while (preguntaDAO.listar().size() > cont){

                    if(preguntaDAO.listar().get(cont).getTematica().equals(tematica)){
                        System.out.println("Pregunta número " + (cont + 1));

                        System.out.println(preguntaDAO.listar().get(cont).getTexto());
                        for(int i=0; i<preguntaDAO.listar().get(cont).getOpcion().size(); i++){
                            System.out.println((i + 1) + ") " + preguntaDAO.listar().get(cont).getOpcion().get(i).getTexto());
                        }
            ...
            System.out.println("El total de aciertos son: " + contAciertos);
            Thread.sleep(1000);

            System.out.println("Fin del juego, para empezar otro modo, reinicie la partida");
